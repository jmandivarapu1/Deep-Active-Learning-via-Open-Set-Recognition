...
def main(args):
    ...
    environment=args.environment + '_'+str(args.num_skip_classes)
    cfg = {"server": "jmandivarapu1@retina.cs.gsu.edu","port": 8097}
    vis = visdom.Visdom('http://' + cfg["server"], port = cfg["port"])
    vis.delete_env(args.environment + '_'+str(args.num_skip_classes)) #If you want to clear all the old plots for this python Experiments.Resets the Environment
    ...
    if args.dataset == 'cifar10':
        ...
        data_indexes=Dictlist()
        for a,target,index in train_dataset:data_indexes[target]=index
    ...
    '''
    The below block removes all the random intial 10 or 20 classes from the intial
    labeled pool as specified in the arguments yaml file
    '''
    skip_class_indexes = []
    # Set a length of the list to 10
    for i in range(0, args.num_skip_classes):
        # any random numbers from 0 to 1000
        skip_class_indexes.extend(data_indexes[random.randint(0, 99)])

    #removing the classes indexes from all indexes
    all_indices= np.setdiff1d(list(all_indices), skip_class_indexes)
    #Rest goes normally
    ...
    for split in splits:
        ...
        '''
        Now add back all the skipped classes intially to the set back again
        '''
        unlabeled_indices=list(unlabeled_indices)+list(skip_class_indexes)
        #rest goes asusual
